...
contract Users {
     ...
     */
    function registerUser(string memory _userName, uint _timeBank, bytes32 _status) public
    returns(uint)
    {
    	return addUser(msg.sender, _userName, _timeBank, _status);
    }
     ...
     */
    function addUser(address _wAddr, string memory  _userName, uint _timeBank, bytes32 _status) private
    returns(uint)
    {
     ...
     */
    function updateUser(string memory _newUserName, uint _newTimeBank, bytes32 _newStatus) checkSenderIsRegistered public
    returns(uint)
    {
